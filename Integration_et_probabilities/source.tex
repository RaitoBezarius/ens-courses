...
\begin{align*}
0 &= \int f dm \\
&\geq \int \varepsilon 1 _{ \lbrace f \geq \varepsilon \rbrace } dm \\
&=\varepsilon m (f \geq \varepsilon )
\end{align*}

Donc $\forall \varepsilon > 0 , m( f\geq \varepsilon ) = 0$ 

Troisième point

on prend l'intersection sur $\varepsilon = \frac{1}{n}$

$\Rightarrow m ( f>0) = 0 $ car $\lbrace f \geq 0}$

Quatrième point :

Si $g$ est simple sur $\Omega$, 

\begin{align*}
\int_A f dm_A = \sum \limits_{y \in \Im y} y m_A (g=y) = \sum \limits_y y m (A \cap \lbrace \rbrace $
\end{align*}

\end{demo}

\begin{theorem}[Théorème de convergence monotone]
Si $f_n$ est une suite de fonctions mesurables positives coissante, alors :

$$\int f_n \rightarrow \int \liminf f_n dm $$

\end{theorem}

On peut voir ce théoème comme un analogue du théorème de convergence donnée au début du cours. On pourrait se ramener au truc précédent en disant que les intégrales positives sont les trucs étudiés, mais nous ne nous sommes pas encore donné un cadre rigoureux pour cela.

\begin{demo}
$f_n \leq f$, donc $\forall n, \ \int f_M \leq f$, donc $\lim \int f_n \leq \int f $.

Il faut démontrer que $\lim \int f_n \geq f$.

On se fixe $\lambda \in ]0,1[, \lambda g $ et on définit une suite de fonction 

$g_n := \lambda g $ si $f_n \geq \lambda y $ et vaut 0 si $f_n < \lambda y$.

$g_n \leq f_n , g_n$ est croissante et $g_n \rightarrow \lambda y$

On avait démontré le théorème de convergence monotone pour les fonctions simples.   

On a montré que $\forall \lambda \in ]0, 1[$, $\lim \int f_n \geq \int \lambda y = \lambda \int g $, en prenant le sup sur $\lambda$,

$$ \forall $$
\end{demo}

\begin{theorem}[Lemme de fatou]
Soit $f_n : \Omega \rightarrow [0, + \infty ] $ une suite de fonctions mesurables. Alors $\int \liminf f_n \leq \liminf \int f_n $.
\end{theorem}

\begin{rem}
\begin{enumerate}
\item Voir le lemme de Fatou ensembliste;
\item Sur $\R$, $f_n (x) =n \left( \dfrac{1}{n e^(-nx)^2} \right) $ (on prendre la fonction chapeau).
\end{enumerate}

Ce dernier point fonctionne car on a choisit la bonne mesure de scaling. 
\end{rem}

\begin{demo}
$g_n = \inf \limits_{k \geq n } f_k$

$g_n$ est croissante, et en posant $f :=\liminf f_n$, par le théorème de convergence monotone, 

$\int g_n \rightarrow \int f$

et $g_n \leq f_n$ donc  $\liminf \int f_n \geq \lim \int g_n = \int f $.
\end{demo}
